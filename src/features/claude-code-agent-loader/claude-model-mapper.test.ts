...
describe("mapClaudeModelToOpenCode", () => {
  ...
  describe("#given Claude Code alias", () => {
    it("#when called with sonnet #then maps to github-copilot claude-sonnet-4-6 object", () => {
    ...
    it("#when called with haiku #then maps to github-copilot claude-haiku-4-5 object", () => {
      expect(mapClaudeModelToOpenCode("haiku")).toEqual({ providerID: "github-copilot", modelID: "claude-haiku-4-5" })
    })
  ...
  describe("#given bare Claude model name", () => {
    it("#when called with claude-sonnet-4-5-20250514 #then adds github-copilot object format", () => {
    ...
    it("#when called with claude-haiku-4-5-20251001 #then adds github-copilot object format", () => {
      expect(mapClaudeModelToOpenCode("claude-haiku-4-5-20251001")).toEqual({ providerID: "github-copilot", modelID: "claude-haiku-4-5-20251001" })
    })

    it("#when called with claude-3-5-sonnet-20241022 #then adds github-copilot object format", () => {
      expect(mapClaudeModelToOpenCode("claude-3-5-sonnet-20241022")).toEqual({ providerID: "github-copilot", modelID: "claude-3-5-sonnet-20241022" })
    })
  })
  ...
  describe("#given model with dot version numbers", () => {
    it("#when called with claude-3.5-sonnet #then normalizes dots and returns object format", () => {
      expect(mapClaudeModelToOpenCode("claude-3.5-sonnet")).toEqual({ providerID: "github-copilot", modelID: "claude-3-5-sonnet" })
    })
  ...
  describe("#given model already in provider/model format", () => {
    it("#when called with anthropic/claude-sonnet-4-6 #then rewrites to github-copilot subscription", () => {
      expect(mapClaudeModelToOpenCode("anthropic/claude-sonnet-4-6")).toEqual({ providerID: "github-copilot", modelID: "claude-sonnet-4-6" })
    })

    it("#when called with anthropic/claude-3.5-sonnet #then normalizes dots and rewrites provider to github-copilot", () => {
      expect(mapClaudeModelToOpenCode("anthropic/claude-3.5-sonnet")).toEqual({ providerID: "github-copilot", modelID: "claude-3-5-sonnet" })
    })
  ...
  describe("#given model with leading/trailing whitespace", () => {
    it("#when called with padded string #then trims before returning object format", () => {
      expect(mapClaudeModelToOpenCode("  claude-sonnet-4-6  ")).toEqual({ providerID: "github-copilot", modelID: "claude-sonnet-4-6" })
    })
  })
})
